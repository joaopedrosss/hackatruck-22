...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    //5 - prefedinir uma localicazacao
    
    func predefinirLocalizacao(){
        let novaCoord = CLLocationCoordinate2D(latitude: -9.5586864, longitude: -35.7473483)
        

        let novaAnot = MKPointAnnotation()
        novaAnot.coordinate = novaCoord
        novaAnot.title = "Shopping"
        
        novaAnot.subtitle = String(describing: "lat: \(novaCoord.latitude) long: \(novaCoord.longitude)")
        
        mapa.addAnnotation(novaAnot)
